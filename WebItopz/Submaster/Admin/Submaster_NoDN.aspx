<%@ Page Language="VB" MasterPageFile="~/Site.Master" AutoEventWireup="false" CodeBehind="Submaster_NoDN.aspx.vb" Inherits="WebItopz.Submaster_Admin_NoDN" title="NoDN" %>
    ...
    <div class="x_panel">
        ...
        <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AutoGenerateColumns="False" OnPreRender="GridView1_PreRender"
            ...
            <Columns>
                ...
                <asp:HyperLinkField DataNavigateUrlFields="LocalMOID" DataNavigateUrlFormatString="NoDN.aspx?ID={0}&amp;action=Redo"
                    HeaderText="Redo" Text="Redo" />
                <asp:HyperLinkField DataNavigateUrlFields="LocalMOID" DataNavigateUrlFormatString="NoDN.aspx?ID={0}&amp;action=Refund"
                    HeaderText="Refund" Text="Refund" />
                <asp:HyperLinkField DataNavigateUrlFields="LocalMOId" DataNavigateUrlFormatString="NoDN.aspx?ID={0}&amp;action=Cancel"
                    HeaderText="Cancel" Text="Cancel" />
                <asp:HyperLinkField DataNavigateUrlFields="LocalMOId" DataNavigateUrlFormatString="NoDN.aspx?ID={0}&amp;action=Success"
        ...
        <asp:GridView ID="GridView2" runat="server" AutoGenerateColumns="False" BackColor="White" OnPreRender="GridView2_PreRender"
            ...
            ForeColor="Black" GridLines="Vertical" Style="margin: 10px" AllowPaging="True" CssClass="datatable-responsive table table-striped table-bordered dt-responsive">
            <FooterStyle BackColor="#CCCC99" />
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
            SelectCommand="SELECT LocalMOID, ReloadMSISDN, ReloadAmount, Robin, ReloadTelco, AgentID, MSISDN, MessageTS, LastupdatedTS, Retry, Status FROM TxIn WHERE ((Status = @Status) Or (Status = 'PROCESSED')) AND (DNReceivedID = '') AND (DATEDIFF(n, LastUpdatedTS, GETDATE()) > 2) ORDER BY MessageTS DESC">
            <SelectParameters>
