<%@ Page Language="VB" MasterPageFile="~/Site.Master" AutoEventWireup="false" CodeBehind="Submaster_ReportAgentTopup.aspx.vb" Inherits="WebItopz.Submaster_Admin_ReportAgentTopup" title="ReportAgentTopup" %>
    ...
    <div class="col-md-6 col-sm-6 col-xs-12" style="padding-left: 0;">
        <div class="x_panel">
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                SelectCommand="SELECT CONVERT (varchar(10), t.CreatedTS, 111) AS Date, SUM(t.Amount) AS Amount, t.AgentID, i.Name FROM AgentTopupTx AS t INNER JOIN AgentInfo AS i ON t.AgentID = i.AgentID  right join (select AgentID as AI, Agentlvl from AgentInfo where Agentlvl like @Agentlvl+@agentid+'|%'  ) AS c on c.AI = t.AgentID  WHERE (DATEPART(mm, t.CreatedTS) = @month) AND (DATEPART(yyyy, t.CreatedTS) = @year) AND (DAY(t.CreatedTS) = @Day) AND (t.Type = 'TOPUP' OR T.TYPE='RECEIVED') GROUP BY CONVERT (varchar(10), t.CreatedTS, 111), t.AgentID, i.Name ORDER BY CONVERT (INT, t.AgentID)">
                <SelectParameters>
    ...
    <div class="col-md-6 col-sm-6 col-xs-12" style="padding-right: 0;">
        <div class="x_panel">
            ...
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                SelectCommand="SELECT Month(CreatedTS) as Month, Sum(Amount) as Amount FROM [AgentTopupTx]&#13;&#10; right join (select AgentID as AI, Agentlvl from AgentInfo where Agentlvl like @Agentlvl+@agentid+'|%'  ) AS c on c.AI = [AgentTopupTx].AgentID   where year(CreatedTS) = @year&#13;&#10;group by month(CreatedTS) order by month">
                <SelectParameters>
                    <asp:ControlParameter ControlID="DDLYear" Name="year" PropertyName="SelectedValue" />
									 <asp:SessionParameter Name="Agentlvl" SessionField="AGENT_AGENTLVL" />
									 <asp:SessionParameter Name="agentid" SessionField="AGENT_ID" />
