...
Partial Class Submaster_TelcoFailReload
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		If sID <> "" Then
			...
			If bWorking Then
				If Request("action").ToUpper = "REDO" Then
					If oCommon.Redo(sID) Then
						Me.lblErrorMsg.Text = "Redoing"
					Else
						' Me.lblErrorMsg.Text = "Redoing Error!!!"
					End If
				ElseIf Request("action").ToUpper = "REFUND" Then
					If oCommon.Refund(sID) Then
						If oCommon.RefundToAgent(sID) Then
							Me.lblErrorMsg.Text = "Refunding"
						Else
							Me.lblErrorMsg.Text = "Refunding Error(2)!!!"
						End If
					Else
						Me.lblErrorMsg.Text = "Refunding Error(1)!!!"
					End If
				ElseIf Request("action").ToUpper = "CANCEL" Then
					If oCommon.Cancel(sID) Then
						Me.lblErrorMsg.Text = "Canceling....!!!"
					Else
						Me.lblErrorMsg.Text = "Canceling.... Error(1)!!!"
					End If
				ElseIf Request("action").ToUpper = "SUCCESS" Then
					If oCommon.Success(sID) Then
						Me.lblErrorMsg.Text = "Marked Success....!!!"
					Else
						Me.lblErrorMsg.Text = "Error Marking Success. Try again!!!"
					End If
				End If
			End If
		Else
	...
	'            sSQL = "UPDATE TxIn SET retry = 0, DNReceivedID='', Status='PENDING', SIMBalanceDeducted='NO' " & _
	'                   "WHERE LocalMOID=@LocalMOID "
	'        Else
	'            sSQL = "UPDATE TxIn SET retry = 0, DNReceivedID='', Status='PENDING'  " & _
	'                   "WHERE LocalMOID=@LocalMOID "
	'        End If
	...
	'    Try
	'        Success = False
	'        sSQL = "UPDATE TxIn SET Status='SUCCESS', TelcoFailReloadFlag=1 " & _
	'               "WHERE LocalMOID=@LocalMOID "
	...
	''                sSQL = "UPDATE AgentBalance SET MaxisBalance=MaxisBalance+@Amount WHERE AgentId=@AgentId "
	''            ElseIf sReloadTelco = "DIGI" Then
	''                sSQL = "UPDATE AgentBalance SET DiGiBalance=DiGiBalance+@Amount WHERE AgentId=@AgentId "
	''            ElseIf sReloadTelco = "CELCOM" Then
	''                sSQL = "UPDATE AgentBalance SET CelcomBalance=CelcomBalance+@Amount WHERE AgentId=@AgentId "
	''            End If
	...
	''            oCmdUpdate.Connection = oConn
	''            oCmdUpdate.Parameters.Clear()
	''            oCmdUpdate.Parameters.Add(New SqlParameter("@Amount", sReloadAmount))
	''            oCmdUpdate.Parameters.Add(New SqlParameter("@AgentId", sAgentId))
	''            oCmdUpdate.ExecuteNonQuery()
