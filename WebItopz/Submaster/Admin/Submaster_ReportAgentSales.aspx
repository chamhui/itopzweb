<%@ Page Language="VB" MasterPageFile="~/Site.Master" AutoEventWireup="false" CodeBehind="Submaster_ReportAgentSales.aspx.vb" Inherits="WebItopz.Submaster_ReportAgentSales" title=" ReportAgentSales" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                    SelectCommand="SELECT sum([ReloadAmount]) as Total FROM [TxIn] right join (select AgentID as AI, Agentlvl from AgentInfo where Agentlvl like @Agentlvl+@currentagentid+'|%'  ) AS c on c.AI = @AgentId WHERE ((left([MessageTS],8) >= @MessageTS) AND (left([MessageTS],8) <= @MessageTS1) AND ([Status] = @Status)) and AgentId=@AgentId">
                    <SelectParameters>
                        <asp:ControlParameter ControlID="txtDateFrom" Name="MessageTS" PropertyName="Text"
                            Type="String" />
                        ...
                        <asp:ControlParameter ControlID="txtAgentID" DefaultValue="" Name="AgentId" PropertyName="Text" />
									 ...
									 <asp:SessionParameter Name="currentagentid" SessionField="AGENT_ID" />
                    </SelectParameters>
                </asp:SqlDataSource>
            <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                    SelectCommand="SELECT ReloadTelco , sum([ReloadAmount]) as Total FROM [TxIn] right join (select AgentID as AI, Agentlvl from AgentInfo where Agentlvl like @Agentlvl+@currentagentid+'|%'  ) AS c on c.AI = @AgentId WHERE ((left([MessageTS],8) >= @MessageTS) AND (left([MessageTS],8) <= @MessageTS1) AND ([Status] = @Status)) and AgentId=@AgentId&#13;&#10;group by ReloadTelco ">
                    <SelectParameters>
                        <asp:ControlParameter ControlID="txtDateFrom" Name="MessageTS" PropertyName="Text" />
    ...
    <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
        SelectCommand="SELECT FROM TxReload INNER JOIN TxIn ON TxReload.LocalMOID COLLATE Chinese_Taiwan_Stroke_CI_AS = TxIn.LocalMOID ">
                ...
                <asp:SqlDataSource ID="SqlDataSource6" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                    SelectCommand="SELECT MSISDN , ReloadTelco, sum([ReloadAmount]) as Total FROM [TxIn] right join (select AgentID as AI, Agentlvl from AgentInfo where Agentlvl like @Agentlvl+@currentagentid+'|%'  ) AS c on c.AI = @AgentId  WHERE ((left([MessageTS],8) >= @MessageTS) AND (left([MessageTS],8) <= @MessageTS1) AND ([Status] = @Status)) and AgentId=@AgentId GROUP BY MSISDN, ReloadTelco&#13;&#10;order by MSISDN">
                    <SelectParameters>
                        <asp:ControlParameter ControlID="txtDateFrom" Name="MessageTS" PropertyName="Text" />
                        <asp:ControlParameter ControlID="txtDateTo" Name="MessageTS1" PropertyName="Text" />
                        <asp:Parameter DefaultValue="SUCCESS" Name="Status" Type="String" />
