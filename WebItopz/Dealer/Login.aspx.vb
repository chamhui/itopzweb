...
Partial Class DealerLogin
	...
	Private Function isValidLogin(ByVal sLogin As String, ByVal sPassword As String) As Boolean
		...
		Try
			...
			sSQL = "SELECT am.AgentID, ai.Name,Agentlvl FROM AgentMSISDN am , AgentInfo ai WHERE am.agentid=ai.agentid AND am.MSISDN=@MSISDN AND am.PASSWORD=@PASSWORD AND am.STATUS=@STATUS and (ai.Retailer is null) and (((ai.ParentAgentID=0) AND (Agentlvl IS NULL)) or ((Agentlvl like '%|1|%') AND (ai.ParentAgentID > 0)))"
			...
			If oDR.Read Then
				...
				sName = oDR.Item("Name")
				Session("DEALER_ID") = sAgentID
				Session("DEALER_NAME") = sName
				Session("DEALER_MSISDN") = sLogin
				Try
					Session("AGENT_AGENTLVL") = oDR.Item("Agentlvl")
				Catch ex As Exception
					Session("AGENT_AGENTLVL") = ""
				End Try
