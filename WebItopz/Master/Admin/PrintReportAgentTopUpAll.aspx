<%@ Page Language="VB" AutoEventWireup="false" CodeBehind="PrintReportAgentTopUpAll.aspx.vb" Inherits="WebItopz.PrintReportAgentTopUpAll" %>
    ...
    <div style="margin: 10px;">
        ...
        <div class="item form-group form-horizontal col-md-12 col-sm-12 col-xs-12">
            <div class="col-md-12 col-sm-12 col-xs-12">
                ...
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>" SelectCommand="SELECT Amount , BankIn, Remarks, AgentID, CreatedTS FROM AgentTopupTx WHERE (REPLACE(CONVERT (char(10), CreatedTS, 111), '/', '') >= @CreatedTS) AND (REPLACE(CONVERT (char(10), CreatedTS, 111), '/', '') <= @CreatedTS2)">
                    <SelectParameters>
        ...
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
            SelectCommand="SELECT Sum(Amount) as Amount  FROM [AgentTopupTx] WHERE (REPLACE(CONVERT (char(10), CreatedTS, 111), '/', '') >= @CreatedTS) AND (REPLACE(CONVERT (char(10), CreatedTS, 111), '/', '') <= @CreatedTS2)&#13;&#10;">
            <SelectParameters>
                <asp:ControlParameter ControlID="txtDateFrom" Name="CreatedTS" PropertyName="Text" />
                <asp:ControlParameter ControlID="txtDateTo" Name="CreatedTS2" PropertyName="Text" />
            </SelectParameters>
