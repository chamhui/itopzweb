<%@ Page Language="VB" MasterPageFile="~/Site.Master" AutoEventWireup="false" CodeBehind="ReportSalesByDenoSummary.aspx.vb" Inherits="WebItopz.ReportSalesByDenoSummary" title="Report Sales By Deno Summary" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12" style="padding-left: 0;">
        <div class="x_panel">
            ...
            <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource1" OnPreRender="GridView1_PreRender"
                ...
                GridLines="Vertical" BorderStyle="None" class="table table-striped table-bordered">
                <Columns>
                    <asp:BoundField DataField="Date" HeaderText="Date" SortExpression="Date" />
                    <asp:BoundField DataField="Deno" HeaderText="Deno$" SortExpression="ReloadAmount" />
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                SelectCommand="SELECT month(CreatedTS) AS Date, ReloadTelco, stuff(substring(MessageIn, 10, len(MessageIn)), 1, patindex('%[0-9]%', substring(MessageIn, 10, len(MessageIn)))-1, '') as Deno, COUNT(*) AS Qty, sum(ReloadAmount) as Total FROM [TxIn]WHERE ((Status = 'SUCCESS') or  (Status = 'PENDING')) and (ReloadTelco='M1') and datepart(mm,createdts)=@month and datepart(yyyy,createdts)=@year&#13;&#10;group by month(CreatedTS), ReloadTelco, substring(MessageIn, 10, len(MessageIn)) order by Date, ReloadTelco, stuff(substring(MessageIn, 10, len(MessageIn)), 1, patindex('%[0-9]%', substring(MessageIn, 10, len(MessageIn)))-1, '') ">
                <SelectParameters>
            ...
            <p>Singtel</p>
            <asp:GridView ID="GridView3" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource3" OnPreRender="GridView1_PreRender"
                BackColor="White" BorderColor="#DEDFDE" BorderWidth="1px" CellPadding="4" ForeColor="Black" 
                GridLines="Vertical" BorderStyle="None" class="table table-striped table-bordered">
                <Columns>
                    <asp:BoundField DataField="Date" HeaderText="Date" SortExpression="Date" />
            ...
            <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                SelectCommand="SELECT month(CreatedTS) AS Date, ReloadTelco, stuff(substring(MessageIn, 10, len(MessageIn)), 1, patindex('%[0-9]%', substring(MessageIn, 10, len(MessageIn)))-1, '') as Deno, COUNT(*) AS Qty, sum(ReloadAmount) as Total FROM [TxIn]WHERE ((Status = 'SUCCESS') or  (Status = 'PENDING')) and (ReloadTelco='Singtel') and datepart(mm,createdts)=@month and datepart(yyyy,createdts)=@year&#13;&#10;group by month(CreatedTS), ReloadTelco, substring(MessageIn, 10, len(MessageIn)) order by Date, ReloadTelco, stuff(substring(MessageIn, 10, len(MessageIn)), 1, patindex('%[0-9]%', substring(MessageIn, 10, len(MessageIn)))-1, '')">
                <SelectParameters>
                    <asp:ControlParameter ControlID="DDLMonth" Name="month" PropertyName="SelectedValue" />
                    <asp:ControlParameter ControlID="DDLYear" Name="year" PropertyName="SelectedValue" />
                </SelectParameters>
            ...
            <asp:GridView ID="GridView4" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource4" OnPreRender="GridView1_PreRender"
                BackColor="White" BorderColor="#DEDFDE" BorderWidth="1px" CellPadding="4" ForeColor="Black" 
                GridLines="Vertical" BorderStyle="None" class="table table-striped table-bordered">
                <Columns>
                    <asp:BoundField DataField="Date" HeaderText="Date" SortExpression="Date" />
            ...
            <asp:SqlDataSource ID="SqlDataSource4" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                SelectCommand="SELECT month(CreatedTS) AS Date, ReloadTelco, stuff(substring(MessageIn, 10, len(MessageIn)), 1, patindex('%[0-9]%', substring(MessageIn, 10, len(MessageIn)))-1, '') as Deno, COUNT(*) AS Qty, sum(ReloadAmount) as Total FROM [TxIn]WHERE ((Status = 'SUCCESS') or  (Status = 'PENDING')) and (ReloadTelco like '%Starhub%') and datepart(mm,createdts)=@month and datepart(yyyy,createdts)=@year&#13;&#10;group by month(CreatedTS), ReloadTelco, substring(MessageIn, 10, len(MessageIn)) order by Date, ReloadTelco, stuff(substring(MessageIn, 10, len(MessageIn)), 1, patindex('%[0-9]%', substring(MessageIn, 10, len(MessageIn)))-1, '')">
                <SelectParameters>
                    <asp:ControlParameter ControlID="DDLMonth" Name="month" PropertyName="SelectedValue" />
                    <asp:ControlParameter ControlID="DDLYear" Name="year" PropertyName="SelectedValue" />
                </SelectParameters>
    ...
    <div class="col-md-6 col-sm-6 col-xs-12" style="padding-right: 0;display:none;">
        <div class="x_panel">
            <div class="x_title">
            ...
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                SelectCommand="SELECT substring(MessageIn, 10, len(MessageIn)) as Deno, month(CreatedTS) AS Month, ReloadTelco, ReloadAmount, COUNT(*) AS Qty, COUNT(*)*ReloadAmount as total FROM [TxIn] where month(createdts)=@month and year(CreatedTS) = @year and status = 'success' Group By ReloadTelco, ReloadAmount, month(CreatedTS), substring(MessageIn, 10,len(MessageIn)) order by ReloadTelco, ReloadAmount ">
                <SelectParameters>
                    <asp:ControlParameter ControlID="DDLYear" Name="year" PropertyName="SelectedValue" />
                        <asp:ControlParameter ControlID="DDLMonth" Name="month" PropertyName="SelectedValue" />
                </SelectParameters>
