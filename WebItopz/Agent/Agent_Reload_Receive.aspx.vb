...
Partial Class Agent_Reload_Receive
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		If Not Page.IsPostBack Then
			getNote("SINGTEL")
			txtdeno.Text = Request.QueryString("deno")
			If (Request.QueryString("telco") = "starhubdata") Then
				txtTelco.Text = "STARHUB DATA"
			Else

				txtTelco.Text = Request.QueryString("telco").ToUpper
			End If
			txtReloadAmount.Text = Request.QueryString("value")
			If (Request.QueryString("value") = 0) Then
				txtReloadAmount.Enabled = True
			End If
			Me.lblErrorMsg.Text = ""
	...
	Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
		...
		Else
			...
			Me.lblReloadAmount.Text = "SGD " & txtReloadAmount.Text
			Me.lblReloadTelco.Text = Me.txtTelco.Text
			Me.txtTelco.Enabled = False
		End If
	...
	Private Function InsertIntoTxIn(ByVal sMSISDN As String, ByVal sMOMsg As String, ByVal sMOTS As String, ByVal sDeno As String) As Boolean
		...
		Try
			...
			If UBound(aMOMsg) = 1 Then
				...
				sReloadTelco = txtTelco.Text  'GetPrefix("6" & sReloadMSISDN)
			...
			getsReloadAmount(sDeno, sReloadTelco, oConn, sReloadAmount)
			If sAgentId = "" Then
				bValid = False
			...
			If CheckLastReloadTS(sReloadMSISDN, sDeno, sReloadTelco, oConn) Then
				bValid = False
				sErrorCode = "Duplication Reload Submitted"
			End If
			If (sDeno = "0") Then
				sDeno = sReloadAmount.ToString()
			End If

			sProductID = getProductID(sReloadTelco)
			sRebateID = GetRebateID(sDeno, sProductID, oConn)
			'Me.lblReloadTelco.Text = "sRebateID=" & sRebateID
			If (sReloadTelco.ToUpper = "STARHUB DATA") Then
				sMOMsg = txtReloadMSISDN.Text & ".HD" & txtdeno.Text
			ElseIf (txtdeno.Text = "0") Then
				sMOMsg = txtReloadMSISDN.Text & " " & txtReloadAmount.Text
			Else
				sMOMsg = txtReloadMSISDN.Text & " " & txtdeno.Text
			End If
			'If (sReloadTelco = "SINGTEL") Then
			'	sStatus = getSingtelStatus(sAgentId, sStatus)
			'End If
			...
			If (sReloadTelco = "SINGTEL") Or (sReloadTelco = "STARHUB") Or (sReloadTelco = "STARHUB DATA") Then
				iRobin = getSingtelRobin(sAgentId, iRobin)
				'iRobin = getSingtelRobin7(sAgentId, iRobin)
			End If
			If (sReloadTelco = "SINGTEL") Then
				iRobin = getSingtelRobin7(sAgentId, iRobin)
			End If
			'If ((sReloadTelco = "STARHUB") Or (sReloadTelco = "STARHUB DATA")) And ((sAgentId = "1869") Or (sAgentId = "1988") Or (sAgentId = "1676") Or (sAgentId = "1523")) Then
			'	iRobin = 7
			'End If
	...
	End Function
	Private Function getSingtelStatus(ByVal sAgent As String, ByVal sStatus As String)
		Dim sSQL As String = ""
		' oConn As New SqlConnection(ConfigurationManager.ConnectionStrings("RichTechConnectionString").ConnectionString)
		Dim oConn As New SqlConnection(ConfigurationManager.ConnectionStrings("RichTechConnectionString").ConnectionString)
		Dim oCmd As New SqlCommand
		Dim oDR As SqlDataReader = Nothing
		Dim sID As String = "8"
		Dim sID2 As String = "8"
		getSingtelStatus = sStatus

		Try
			oConn.Open()
			oCmd = oConn.CreateCommand
			oCmd.Connection = oConn
			sSQL = "SELECT * FROM  AgentInfo where (ParentAgentID=@sID or AgentID=@sID or AgentID=@sID2) and (AgentID=@AgentID)"

			oCmd.CommandText = sSQL
			With oCmd.Parameters
				.Clear()
				.AddWithValue("@AgentID", sAgent)
				.AddWithValue("@sID", sID)
				.AddWithValue("@sID2", sID2)
			End With
			oDR = oCmd.ExecuteReader

			If oDR.Read() Then
				getSingtelStatus = "BLOCKED"
			End If
			oDR.Close()
			oConn.Close()
		...
		End Try
	End Function
	Private Function getSingtelRobin(ByVal sAgent As String, ByVal iRobin As Integer)
		Dim sSQL As String = ""
		Dim oConn As New SqlConnection(ConfigurationManager.ConnectionStrings("RichTechConnectionString").ConnectionString)
		Dim oCmd As New SqlCommand
		Dim oDR As SqlDataReader = Nothing
		Dim sID As String = "2"
		Dim sID2 As String = "1976"
		getSingtelRobin = iRobin
	...
	Private Function getSingtelRobin7(ByVal sAgent As String, ByVal iRobin As Integer)
		Dim sSQL As String = ""
		Dim oConn As New SqlConnection(ConfigurationManager.ConnectionStrings("RichTechConnectionString").ConnectionString)
		Dim oCmd As New SqlCommand
		Dim oDR As SqlDataReader = Nothing
		Dim sID As String = "1982"

		getSingtelRobin7 = iRobin

		Try
			oConn.Open()
			oCmd = oConn.CreateCommand
			oCmd.Connection = oConn
			sSQL = "SELECT * FROM  AgentInfo where (AgentID=@sID) and (AgentID=@AgentID)"
	...
	Private Function getsReloadAmount(ByVal sDeno As String, ByVal p_telco As String, ByVal oConn As SqlConnection, ByRef sReloadAmount As Integer) As Integer
		Dim sSQL As String = ""
		...
		Try
			...
			oCmd = oConn.CreateCommand
			oCmd.Connection = oConn
			sSQL = "SELECT * FROM ProductRebate right join (SELECT ProductID,Telco as P_Telco from ProductInfo) pi on ProductRebate.ProductID = pi.ProductID WHERE Denomination=@P_Deno and P_Telco=@P_Telco and P_FaceValue is not null"

			oCmd.CommandText = sSQL
			With oCmd.Parameters
				.Clear()
				.AddWithValue("@P_Deno", sDeno)
				.AddWithValue("@P_Telco", p_telco)
			End With
			oDR = oCmd.ExecuteReader

			If oDR.HasRows Then
				oDR.Read()
				sReloadAmount = oDR.Item("P_FaceValue")
			End If
			oDR.Close()
			'oConn.Close()

			Return sReloadAmount = sReloadAmount

		Catch ex As Exception
			sReloadAmount = ""
			Return sReloadAmount
	...
	Private Function CheckLastReloadTS(ByVal sReloadMSISDN As String, ByVal sReloadAmount As String, ByVal sReloadTelco As String, ByVal oConn As SqlConnection) As Boolean
		...
		Try
			oCmd = oConn.CreateCommand
			oCmd.Connection = oConn
			sSQL = "SELECT LocalMOID FROM TxIn WHERE ReloadTelco NOT LIKE '%PIN' and (MessageIn like @ReloadMSISDN+' '+@ReloadAmount or MessageIn like @ReloadMSISDN+'.S'+@ReloadAmount  or MessageIn like @ReloadMSISDN+'.H'+@ReloadAmount  or MessageIn like @ReloadMSISDN+'.M'+@ReloadAmount  or MessageIn like @ReloadMSISDN+'.HD'+@ReloadAmount) and datediff(n,CreatedTS,getdate()) < '2'  and (STATUS='SUCCESS' or STATUS='PENDING') "
	...
	Private Function getPostRobin(ByVal sReloadTelco As String) As String
		...
		Try
			...
			oCmd = oConn.CreateCommand
			oCmd.Connection = oConn
			sSQL = "SELECT TOP 1 (Robin+1) as Robin FROM TxIn WHERE ReloadTelco=@ReloadTelco AND Robin<>'8' order by CreatedTS desc "
	...
	Protected Sub btnConfirm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnConfirm.Click
		...
		Me.btnEdit.Text = "New"
		InsertIntoTxIn(Session("AGENT_MSISDN"), sMsg, Format(Now(), "yyyyMMddHHmmss"), txtdeno.Text)
