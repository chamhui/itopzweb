...
Partial Class Agent_ReloadPIN
	...
	Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
		...
		If Me.txtReloadMSISDN.Text = "" Or Me.txtReloadMSISDN.Text.Length < 8 Or txtReloadMSISDN.Text.Length > 14 Then
	...
	Private Function CheckLastReloadTS(ByVal sReloadMSISDN As String, ByVal sReloadAmount As String, ByVal sReloadTelco As String, ByVal oConn As SqlConnection) As Boolean
		...
		Try
			oCmd = oConn.CreateCommand
			oCmd.Connection = oConn
			sSQL = "SELECT LocalMOID FROM TxIn WHERE ReloadTelco NOT LIKE '%PIN' and ReloadMSISDN=@ReloadMSISDN and ReloadAmount=@ReloadAmount and datediff(n,CreatedTS,getdate()) < '5'  and (STATUS='SUCCESS' or STATUS='PENDING') "
	...
	Protected Sub DDLTelco_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DDLTelco.SelectedIndexChanged
		...
		Dim i As Integer
		sNote = getNote(DDLTelco.SelectedValue)
		aNote = sNote.Split(",")
		For i = 0 To UBound(aNote)
			Me.DDLReloadAmount.Items.Add(aNote(i))
		Next
	...
	Private Function getNote(ByVal sTelco As String) As String
		...
		Try
			...
			For i = 0 To UBound(aNote)
				If aNote(i).Contains("-") Then
					aNote(i) = Extend(aNote(i), "-")
				End If
				If sNote = Nothing Then
					sNote = aNote(i)
				Else
					sNote = sNote & "," & aNote(i)
				End If
	...
	Private Function Extend(ByVal sValue As String, ByVal sdelimiter As String) As String
	...
	Private Function getFirstTelco() As String
		...
		Try
			...
			sSQL = "SELECT TOP 1 Telco FROM ProductInfo WHERE TELCO LIKE '%PIN' AND STATUS=@STATUS"
			oCmd.CommandText = sSQL
			With oCmd.Parameters
				.Clear()
				.AddWithValue("@STATUS", STATUS_ACTIVE)
