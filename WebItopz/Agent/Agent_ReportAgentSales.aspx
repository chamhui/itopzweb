<%@ Page Language="VB"  MasterPageFile="~/Site.Master"  AutoEventWireup="false" Inherits="WebItopz.Agent_ReportAgentSales" title=" ReportAgentSales" Codebehind="Agent_ReportAgentSales.aspx.vb" EnableEventValidation="false" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <div class="page-title">
        <div class="title_left">
            <h3><asp:Label ID="lblTitle" runat="server" ForeColor="Black"></asp:Label></h3>
        </div>
    </div>
    <div class="x_panel">
        <div class="x_title">
        <h2><asp:Label ID="lblTitle2" runat="server" ForeColor="Black"></asp:Label></h2>
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
            SelectCommand="SELECT T.MSISDN, T.ReloadTelco ,T.ReloadAmount, T.ReloadMSISDN, T.LastUpdatedTS, T.MessageIn, T.Status, R.BalanceAfterReload FROM TxIn AS T INNER JOIN TxReload AS R ON T.LocalMOID = R.LocalMOID COLLATE Chinese_Taiwan_Stroke_CI_AS WHERE (T.AgentID = @AgentID) AND (LEFT (T.MessageTS, 8) >= @MessageTS) AND (LEFT (T.MessageTS, 8) <= @MessageTS2) ORDER BY T.MessageTS DESC">
            <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
        SelectCommand="SELECT FROM TxReload INNER JOIN TxIn ON TxReload.LocalMOID COLLATE Chinese_Taiwan_Stroke_CI_AS = TxIn.LocalMOID">
                ...
                <asp:SqlDataSource ID="SqlDataSource6" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
                    SelectCommand="SELECT MSISDN , ReloadTelco, sum([ReloadAmount]) as Total FROM [TxIn] WHERE ((left([MessageTS],8) >= @MessageTS) AND (left([MessageTS],8) <= @MessageTS1) AND ([Status] = @Status)) and AgentId=@AgentId GROUP BY MSISDN, ReloadTelco&#13;&#10;order by MSISDN">
                    <SelectParameters>
                        <asp:ControlParameter ControlID="txtDateFrom" Name="MessageTS" PropertyName="Text" />
                        <asp:ControlParameter ControlID="txtDateTo" Name="MessageTS1" PropertyName="Text" />
                        <asp:Parameter DefaultValue="SUCCESS" Name="Status" Type="String" />
	...
	<asp:SqlDataSource ID="SqlDataSource7" runat="server" ConnectionString="<%$ ConnectionStrings:RichTechConnectionString %>"
        SelectCommand="SELECT CONVERT (varchar(10), AgentID) + ' ' + Name AS Name, AgentID FROM AgentInfo WHERE (AgentID= @AgentID)">
        <SelectParameters>
